...
class ForgeBackend_Null : public IForgeBackend {
public:
    ForgeBackend_Null() : m_LastCommandCount(0), m_LastVertexCount(0) {}
    virtual ~ForgeBackend_Null() {}
    ...
    virtual void RenderDrawList(const ForgeDrawList* drawList) override {
        if (drawList) {
            m_LastCommandCount = drawList->GetCommandCount();
            m_LastVertexCount = drawList->GetVertexCount();
        }
    }
    ...
    uint32_t GetLastCommandCount() const { return m_LastCommandCount; }
    uint32_t GetLastVertexCount() const { return m_LastVertexCount; }

private:
    uint32_t m_LastCommandCount;
    uint32_t m_LastVertexCount;
};
